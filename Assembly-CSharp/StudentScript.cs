...
public class StudentScript : MonoBehaviour
{
	...
	public RiggedAccessoryAttacher ChessterAttacher;
	...
	public Animation OsanaHairAnim;

	public Projector LiquidProjector;

	public Projector FrostProjector;
	...
	public GameObject ChessterMask;
	...
	public int SleuthTargetsSought;

	public int TeacherID = 90;

	public RiggedAccessoryAttacher WrappingAttacher;
	...
	public Material OriginalBloodMaterial;
	...
	public SubtitleType TaskLineResponseType
	{
		get
		{
			...
			if (StudentManager.Eighties)
			{
				...
				if (StudentManager.CustomMode && ((StudentID > 10 && StudentID < 21) || StudentID == 79))
				{
					flag = true;
				}
			}
	...
	public void Start()
	{
		...
		if (!Started)
		{
			...
			else if (Persona == PersonaType.TeachersPet || Persona == PersonaType.Heroic || Persona == PersonaType.Strict || Persona == PersonaType.LandlineUser || Persona == PersonaType.Dangerous)
			{
			...
			if (!StudentManager.Eighties)
			{
				...
				else if (StudentID == 10)
				{
					...
					else
					{
						...
						else if (GameGlobals.GetRivalEliminations(1) == 6)
						{
							...
							StudentManager.Hangouts.List[10].transform.position = new Vector3(-5f, 0f, -6f);
							StudentManager.Hangouts.List[10].transform.eulerAngles = new Vector3(0f, 180f, 0f);
							StudentManager.Week2Hangouts.List[10].transform.position = new Vector3(-5f, 0f, -6f);
							StudentManager.Week2Hangouts.List[10].transform.eulerAngles = new Vector3(0f, 180f, 0f);
							ScheduleBlock obj12 = ScheduleBlocks[2];
							obj12.destination = "Hangout";
							obj12.action = "Relax";
							obj12.time = 8f;
							RelaxAnim = ThinkAnim;
							if (StudentManager.Students[46] != null)
							{
								Curious = true;
								Spawned = true;
								Crush = 46;
				...
				else if (StudentID == 12)
				{
					...
					OriginalIdleAnim = "f02_gentleIdle_00";
					...
					if (StudentManager.MissionMode)
					{
						OriginalPersona = PersonaType.SocialButterfly;
						Persona = PersonaType.SocialButterfly;
					}
				}
				...
				else if (StudentID == 64 || StudentID == 65)
				{
					...
					if (StudentID == 65)
					{
						if (StudentManager.RobotPhase == 1)
						{
							StudentManager.Clubs.List[65].eulerAngles = new Vector3(0f, -90f, 0f);
							AnimationNames[0] = "f02_carefreeTalk_00";
							AnimationNames[1] = "f02_carefreeTalk_01";
							AnimationNames[2] = "f02_carefreeTalk_02";
						}
						else if (StudentManager.RobotPhase == 2)
						{
							StudentManager.Clubs.List[65].position = new Vector3(15.31f, 8f, 26.83333f);
							StudentManager.Clubs.List[65].eulerAngles = new Vector3(0f, 180f, 0f);
							ClubAnim = "f02_brokenSit_00";
							IdleAnim = BulliedIdleAnim;
							WalkAnim = BulliedWalkAnim;
							Depressed = true;
						}
				...
				if ((StudentID == 6 || StudentID == 11) && DatingGlobals.SuitorProgress == 2)
				{
					...
					obj30.action = "Shoes";
					ScheduleBlock obj31 = ScheduleBlocks[8];
			...
			else
			{
				...
				if (!StudentManager.CustomMode)
				{
					...
					else if (StudentID == 12)
					{
						...
						GameAnim = "f02_lookLeftRight_00_loop";
						PyroUrge = true;
						PyroLimit = 60f;
						if (StudentManager.Week > 2)
						{
					...
					{
						IdleAnim = "idleConfident_01";
						WalkAnim = "walkConfident_01";
					}
					else if (StudentID == 29)
					{
						IdleAnim = "f02_idleGraceful_00";
					...
					{
						IdleAnim = "f02_newIdle_00";
						WalkAnim = "f02_newUnarmedWalk_00";
					}
					else if (StudentID == 31)
					{
						IdleAnim = BulliedIdleAnim;
					...
					{
						IdleAnim = "f02_idleCouncilStrict_00";
						WalkAnim = "f02_walkCouncilStrict_00";
					}
					...
					{
						IdleAnim = "f02_idleChill_01";
						WalkAnim = "f02_walkChill_01";
					}
					else if (StudentID == 54)
					{
						IdleAnim = "f02_idleShy_00_HandsUp";
						WalkAnim = "f02_walkShy_00_HandsUp";
					}
			...
			else if (Club == ClubType.Occult)
			{
				if (!Male)
				{
					WaveAnim = "f02_casualWave_04";
					PatrolAnim = "f02_pondering_00";
					CharacterAnimation[PatrolAnim].speed = 0.7692f;
				}
				else
				{
					PatrolAnim = "pondering_00";
					CharacterAnimation[PatrolAnim].speed = 0.7692f;
				}
			}
			...
			else if (Club == ClubType.Science)
			{
				if (!StudentManager.Eighties)
				{
					...
					else if (StudentID == 65)
					{
						...
						if (StudentManager.RobotPhase == 2)
						{
							ClubAnim = "f02_brokenSit_00";
						}
					}
			...
			else if (OriginalClub == ClubType.Gardening)
			{
				...
				else
				{
					if (Male)
					{
						PatrolAnim = "arrangeFlowers_00";
						ClubAnim = "arrangeFlowers_00";
					}
					else
					{
						PatrolAnim = "f02_arrangeFlowers_00";
						ClubAnim = "f02_arrangeFlowers_00";
					}
					if (StudentID == 72 && StudentManager.SuitorID == StudentID)
					{
						Debug.Log("Suitor boy knows that his RelaxAnim should be Thinking.");
						PatrolAnim = "thinking_00";
						RelaxAnim = "thinking_00";
						ClubAnim = "thinking_00";
					}
					CharacterAnimation[PatrolAnim].speed = 0.9f;
				}
		...
		if (StudentManager.CustomMode)
		{
			if (StudentManager.JSON.Misc.AnimSet[StudentID] < IdleAnims.Length)
			{
				...
				AnimSetID = StudentManager.JSON.Misc.AnimSet[StudentID];
			}
			if (Slave)
			{
				IdleAnim = BrokenAnim;
		...
		if (FrostProjector != null)
		{
			FrostProjector.material.color = new Color(1f, 1f, 1f, 0f);
		}
		...
		else if (!ClubActivity)
		{
			if (!Yandere.Talking)
			{
				...
				if (Yandere.Noticed || StudentManager.YandereDying)
				{
					...
					{
						if (Yandere.TargetStudent.Hips == null)
						{
	...
	private void UpdateRoutine()
	{
		if (Routine)
		{
			...
			{
				Debug.Log("For some reason, " + Name + "'s Phase was greater than their number of ScheduleBlocks.");
				Phase = ScheduleBlocks.Length - 1;
				if (Bullied || StudentID == StudentManager.VictimID)
				{
				...
				if (Leaving)
				{
					...
					ScheduleBlock obj2 = ScheduleBlocks[Phase];
					obj2.destination = "Exit";
					obj2.action = "Stand";
					GetDestinations();
					CurrentDestination = Destinations[Phase];
			...
			if (!Indoors)
			{
				if (Hurry && !Tripped && MustTrip && base.transform.position.z > -50.5f && base.transform.position.x < 6f)
				{
					...
					Routine = false;
					Blind = true;
					if (Clock.GameplayDay == 3 && !BountyCollider.activeInHierarchy)
					{
			...
			else if (Phase < ScheduleBlocks.Length - 1 && isStudentAvailable)
			{
				...
				if (StudentID == 12 && StudentManager.PyroWateringCan.parent != null)
				{
				...
				if (ScheduleBlocks[Phase].destination == "Guard")
				{
					RivalBodyguard = true;
				}
				WitnessBonus = 0;
				...
				if (Rival)
				{
					...
					else if (Clock.Period == 4 && CurrentAction == StudentActionType.Read && !StudentManager.RivalBookBag.BorrowedBook)
					{
						...
						obj5.destination = "Search Patrol";
						obj5.action = "Search Patrol";
						GetDestinations();
						CurrentDestination = Destinations[Phase];
						Pathfinding.target = Destinations[Phase];
					}
					...
					else if (StudentID > 10 && StudentID < 21 && (Schoolwear != 1 || (BikiniAttacher != null && BikiniAttacher.newRenderer != null)))
					{
						...
						if (CurrentAction == (StudentActionType.AtLocker || StudentActionType.ChangeShoes || StudentActionType.SitAndTakeNotes))
						{
							...
							if (SchoolwearUnavailable)
							{
								...
								{
									Debug.Log("...but it's bikini-clad Chigusa. She HAS to go change.");
									GoChange();
								}
							}
							else
				...
				if (Infatuated && Actions[Phase] == StudentActionType.Admire)
				{
					...
					else
					{
						if (CurrentDestination == null)
						{
							Debug.Log("Uh-oh! For some reason, Student #" + StudentID + ", " + Name + ", had a null CurrentDestination.");
							FixNullDestination();
						}
				...
				if (Bullied && Clock.HourTime > 16f)
				{
					...
					ScheduleBlock obj6 = ScheduleBlocks[Phase];
				...
				if (StudentManager.CustomMode)
				{
					if (Actions[Phase] == StudentActionType.Jog && Schoolwear == 1)
					{
						GoChange();
					}
			...
			if (MeetTime > 0f)
			{
				...
				if (!InEvent && Clock.HourTime > MeetTime && !flag && Schoolwear != 0 && Schoolwear != 2)
				{
					if (Follower != null)
					{
						ScheduleBlock obj7 = Follower.ScheduleBlocks[Follower.Phase];
			...
			if (DistanceToDestination > TargetDistance)
			{
				...
				if (Actions[Phase] == StudentActionType.Sleuth)
				{
					...
					else
					{
						...
						if (LockerRoomCheckTimer > 5f)
						{
							...
							{
								Debug.Log("A target was inside of a locker room or bathroom! This Sleuth will now pick another target.");
								GetSleuthTarget();
							}
						}
					}
				...
				else if (Actions[Phase] == StudentActionType.Relax && Curious)
				{
					...
					{
						if (!StudentManager.Students[Crush].Drowned)
						{
							TargetDistance = 3.9f;
						}
						else
				...
				if (Actions[Phase] != StudentActionType.Follow || (Actions[Phase] == StudentActionType.Follow && DistanceToDestination > TargetDistance + 0.1f))
				{
					...
					if (!InEvent)
					{
						if (Pathfinding.speed > 0f)
						{
							if (!Hurry && Pathfinding.speed == WalkSpeed)
							{
								...
								else if (!CharacterAnimation.IsPlaying(WalkAnim))
								{
					...
					if (!Meeting && !GoAway && !InEvent)
					{
						if (Actions[Phase] == StudentActionType.Clean)
						{
							...
							{
								Debug.Log("Hey, Student #" + StudentID + "'s ''CleaningSpot'' was null. Is that a problem?");
								StudentManager.CleaningManager.GetRole(StudentID);
								CleaningSpot = StudentManager.CleaningManager.Spot;
								CleaningRole = StudentManager.CleaningManager.Role;
							}
				...
				if (RivalBodyguard)
				{
					if (StudentManager.LockerRoomArea.bounds.Contains(StudentManager.Students[StudentManager.RivalID].transform.position))
					{
						if (Male)
						{
							CharacterAnimation.CrossFade("impatientWait_00");
					...
					if (!Meeting)
					{
						if (!GoAway)
						{
							if (Actions[Phase] == StudentActionType.AtLocker)
							{
								...
								{
									Debug.Log(Name + " is calling ChangeClothing() from this part of the code.");
									ChangeClothing();
								}
								else
								{
										...
										if (!Male)
										{
											if (!Phoneless)
											{
												if (!flag3)
												{
													if (FollowTarget != null)
													{
														if (FollowTarget.CameraReacting)
														{
													...
													{
														if (FollowTarget != null && FollowTarget.Alive)
														{
							...
							else if (Actions[Phase] == StudentActionType.Relax)
							{
								...
								else
								{
									...
									else
									{
										...
										if (StudentID == 6 && Clock.GameplayDay == 2 && !BountyCollider.activeInHierarchy)
										{
											BountyCollider.SetActive(value: true);
										}
									}
							...
							else if (Actions[Phase] == StudentActionType.SitAndTakeNotes)
							{
								...
								if (MustChangeClothing)
								{
									...
									Debug.Log(Name + "'s goal is to SitAndTakeNotes, so they are going to try to change into their school uniform.");
									Schoolwear = 1;
									ChangeClothing();
								}
								else if (Bullied)
							...
							else if (Actions[Phase] == StudentActionType.ClubAction)
							{
										...
										if ((double)Vector3.Distance(base.transform.position, CurrentDestination.position) < 0.5)
										{
											if (SciencePhase == 0)
											{
												if (!StudentManager.Eighties)
												{
													...
													else if (StudentID == 65 && StudentManager.RobotPhase == 1)
													{
														if (!StudentManager.RobotChan.Hunting)
														{
															CharacterAnimation.CrossFade(RandomAnim);
															if (CharacterAnimation[RandomAnim].time >= CharacterAnimation[RandomAnim].length)
															{
														...
														{
															CharacterAnimation.CrossFade("f02_lookLeftRight_00_loop");
														}
													}
												}
												else
								...
								else if (OriginalClub == ClubType.Sports)
								{
									...
									else if (ClubActivityPhase == 15)
									{
										...
										else if (!StudentManager.CustomMode && CharacterAnimation[ClubAnim].time >= 2f)
										{
											if (Male)
											{
												float value2 = Cosmetic.EightiesGoggles[StudentID].GetComponent<SkinnedMeshRenderer>().GetBlendShapeWeight(1) - Time.deltaTime * 200f;
												Cosmetic.EightiesGoggles[StudentID].GetComponent<SkinnedMeshRenderer>().SetBlendShapeWeight(1, value2);
											}
											else
											{
												float value3 = Cosmetic.EightiesGoggles[StudentID].GetComponent<SkinnedMeshRenderer>().GetBlendShapeWeight(2) - Time.deltaTime * 200f;
												Cosmetic.EightiesGoggles[StudentID].GetComponent<SkinnedMeshRenderer>().SetBlendShapeWeight(2, value3);
											}
										}
									...
									else if (ClubActivityPhase == 16)
									{
										if ((double)CharacterAnimation[ClubAnim].time >= 6.1)
										{
											...
											else if (!StudentManager.CustomMode)
											{
												...
												{
													Cosmetic.MaleHair[Cosmetic.Hairstyle].GetComponent<SkinnedMeshRenderer>().SetBlendShapeWeight(0, 100f);
													Cosmetic.EightiesGoggles[StudentID].GetComponent<SkinnedMeshRenderer>().SetBlendShapeWeight(1, 0f);
												}
												else
												{
													Cosmetic.FemaleHair[Cosmetic.Hairstyle].GetComponent<SkinnedMeshRenderer>().SetBlendShapeWeight(0, 100f);
													Cosmetic.EightiesGoggles[StudentID].GetComponent<SkinnedMeshRenderer>().SetBlendShapeWeight(2, 0f);
												}
											}
									...
									else if (ClubActivityPhase == 19)
									{
										...
										if (CharacterAnimation[ClubAnim].time >= 4.66666f)
										{
											if (!StudentManager.Eighties)
											{
												float value4 = Cosmetic.Goggles[StudentID].GetComponent<SkinnedMeshRenderer>().GetBlendShapeWeight(0) - Time.deltaTime * 200f;
												Cosmetic.Goggles[StudentID].GetComponent<SkinnedMeshRenderer>().SetBlendShapeWeight(0, value4);
											}
											else if (!StudentManager.CustomMode)
											{
												if (Male)
												{
													float value5 = Cosmetic.EightiesGoggles[StudentID].GetComponent<SkinnedMeshRenderer>().GetBlendShapeWeight(1) + Time.deltaTime * 200f;
													Cosmetic.EightiesGoggles[StudentID].GetComponent<SkinnedMeshRenderer>().SetBlendShapeWeight(1, value5);
												}
												else
												{
													float value6 = Cosmetic.EightiesGoggles[StudentID].GetComponent<SkinnedMeshRenderer>().GetBlendShapeWeight(2) + Time.deltaTime * 200f;
													Cosmetic.EightiesGoggles[StudentID].GetComponent<SkinnedMeshRenderer>().SetBlendShapeWeight(2, value6);
												}
											}
										}
										if (CharacterAnimation[ClubAnim].time >= CharacterAnimation[ClubAnim].length)
										{
											...
											else if (!StudentManager.CustomMode)
											{
												if (Male)
												{
													Cosmetic.EightiesGoggles[StudentID].GetComponent<SkinnedMeshRenderer>().SetBlendShapeWeight(1, 100f);
												}
												else
												{
													Cosmetic.EightiesGoggles[StudentID].GetComponent<SkinnedMeshRenderer>().SetBlendShapeWeight(2, 100f);
												}
											}
							...
							else if (Actions[Phase] == StudentActionType.SitAndSocialize)
							{
								...
								else
								{
									...
									if (StudentID == 56 && Clock.GameplayDay == 7)
									{
										BountyCollider.SetActive(value: true);
									}
								}
							...
							else if (Actions[Phase] == StudentActionType.SitAndEatBento)
							{
								...
								else if (!DiscCheck && Alarm < 100f)
								{
									...
									if (!Emetic && !Lethal && !Sedated && !Headache)
									{
										...
										if (FollowTarget != null && ((FollowTarget.CurrentAction != StudentActionType.SitAndEatBento && !FollowTarget.Dying && !FollowTarget.Emetic) || Clock.HourTime > 13.375f))
										{
											if (FollowTarget.Alive)
											{
												...
												obj8.destination = "Follow";
												obj8.action = "Follow";
												ScheduleBlock obj9 = ScheduleBlocks[5];
												obj9.destination = "Follow";
												obj9.action = "Follow";
												GetDestinations();
									...
									else if (Lethal)
									{
										if (!Private)
										{
											SpawnTimeRespectingAudioSource(PoisonDeathClip);
											if (Male)
											{
							...
							else if (Actions[Phase] == StudentActionType.Patrol)
							{
								...
								if (StudentID == 1 && ExtraBento && CurrentDestination == StudentManager.Patrols.List[StudentID].GetChild(0))
								{
									...
									else if (StudentManager.CustomMode)
									{
										...
										obj10.destination = scheduleBlock2.destination;
										obj10.action = scheduleBlock2.action;
							...
							else if (Actions[Phase] == StudentActionType.Cuddle)
							{
								if (Vector3.Distance(base.transform.position, Partner.transform.position) < 1f && Partner.Routine)
								{
									...
									if (CuddlePartnerID == 1)
									{
										...
										{
											CharacterAnimation.CrossFade("f02_cuddle_01");
										}
									}
									else if (!Male)
									{
							...
							else if (Actions[Phase] == StudentActionType.Graffiti)
							{
								...
								if (GraffitiPhase == 0)
								{
									SpawnTimeRespectingAudioSource(BullyGiggles[UnityEngine.Random.Range(0, BullyGiggles.Length)]);
							...
							{
								CharacterAnimation.cullingType = AnimationCullingType.AlwaysAnimate;
								if (StudentManager.Students[StudentManager.VictimID] != null && StudentManager.Students[StudentManager.VictimID].Alive && !StudentManager.Students[StudentManager.VictimID].Tranquil && !StudentManager.Students[StudentManager.VictimID].InEvent)
								{
									...
									else
									{
										...
										if (BullyID == 1)
										{
											if (BullyPhase == 0)
											{
												...
												if (BullyTimer >= 10f && Vector3.Distance(base.transform.position, StudentManager.Students[1].transform.position) > 10f)
												{
											...
											else if (BullyPhase == 2)
											{
												...
												{
													SpawnTimeRespectingAudioSource(BullyLaughs[UnityEngine.Random.Range(0, BullyLaughs.Length)]);
								...
								{
									Debug.Log("This code is called when the bullies' victim is missing or dead or busy with an event.");
									CharacterAnimation.cullingType = AnimationCullingType.BasedOnRenderers;
									DistanceToDestination = 100f;
							...
							else if (Actions[Phase] == StudentActionType.Sulk)
							{
								...
								else
								{
									...
									if (StudentID == 76 && Clock.GameplayDay == 10)
									{
										BountyCollider.SetActive(value: true);
									}
								}
							...
							else if (Actions[Phase] == StudentActionType.Sleuth)
							{
								if (StudentManager.SleuthPhase != 3)
								{
									...
									{
										CharacterAnimation.CrossFade(GenderPrefix + "sleuthScan_00");
										if (CharacterAnimation[GenderPrefix + "sleuthScan_00"].time >= CharacterAnimation[GenderPrefix + "sleuthScan_00"].length)
										{
											CharacterAnimation[GenderPrefix + "sleuthScan_00"].time = 0f;
										}
										SmartPhone.SetActive(value: true);
										if (StudentManager.Eighties)
										{
											SmartPhone.transform.localPosition = new Vector3(0.05f, 0f, 0f);
											SmartPhone.transform.localEulerAngles = new Vector3(0f, 22.5f, 172.5f);
										}
									}
								}
								else
										...
										if (CharacterAnimation[StripAnim].time >= 1.5f)
										{
											...
											if (CharacterAnimation[StripAnim].time > CharacterAnimation[StripAnim].length)
											{
												...
												if (!StudentManager.CommunalLocker.SteamCountdown)
												{
													...
													if (CurrentDestination == null)
													{
														Pathfinding.target = StudentManager.SunbatheSpots[0];
														CurrentDestination = StudentManager.SunbatheSpots[0];
													}
													SunbathePhase++;
												}
											}
							...
							else if (Actions[Phase] == StudentActionType.Miyuki)
							{
								...
								{
									if (StudentID == 37 && Clock.GameplayDay == 6)
									{
										BountyCollider.SetActive(value: true);
									}
							...
							else if (Actions[Phase] == StudentActionType.Lyrics)
							{
								...
								else
								{
									...
									{
										AirGuitar.gameObject.SetActive(value: true);
										AirGuitar.Play();
									}
									if (LyricsTimer > 18f)
									{
							...
							else if (Actions[Phase] == StudentActionType.Sew)
							{
								...
								{
									CharacterAnimation.CrossFade("f02_sewing_00");
								}
								...
								if (StudentID == 8 && Clock.GameplayDay == 4 && !BountyCollider.activeInHierarchy)
								{
									BountyCollider.transform.localPosition = new Vector3(0f, 0f, 0f);
									BountyCollider.GetComponent<BoxCollider>().center = new Vector3(0f, 0.65f, 0.175f);
									BountyCollider.GetComponent<BoxCollider>().size = new Vector3(0.45f, 1.3f, 0.55f);
									BountyCollider.SetActive(value: true);
								}
							}
							...
							else if (Actions[Phase] == StudentActionType.LightFire)
							{
								if (PyroPhase < 2)
								{
									...
									{
										PyroTimer += Time.deltaTime * 2f;
										if (PyroTimer > CharacterAnimation["f02_startFire_00"].length)
										{
											PyroTimer = 0f;
											PyroPhase++;
										}
										else if (PyroTimer > 13f)
										{
										...
										else if (PyroTimer > 11.75f)
										{
											Note.transform.parent = null;
											Note.transform.position -= new Vector3(0f, Time.deltaTime, 0f);
										}
										else if (PyroTimer > 11.5f)
										{
											Lighter.SetActive(value: false);
										}
										else if (PyroTimer > 8f)
										{
											PaperFire.SetActive(value: true);
										}
										else if (PyroTimer > 4.5f)
										{
											Lighter.SetActive(value: true);
										}
										else if (PyroTimer > 1f && !Note.gameObject.activeInHierarchy)
										{
								...
								else if (PyroPhase == 2)
								{
									...
									if (PyroTimer > 60f || (Yandere.transform.position.x < -34.315f && Yandere.transform.position.z > 42.375f && Yandere.transform.position.z < base.transform.position.z) || (TargetedForDistraction && Distractor.transform.position.z > base.transform.position.z && Vector3.Distance(base.transform.position, Distractor.transform.position) < 9f))
									{
										if (Yandere.transform.position.x < -34.315f && Yandere.transform.position.z > 42.375f && Yandere.transform.position.z < base.transform.position.z)
										{
											Debug.Log("Stopped because Player was nearby.");
										}
										if (TargetedForDistraction && Vector3.Distance(base.transform.position, Distractor.transform.position) < 9f)
										{
											Debug.Log("Stopped because Distractor was nearby.");
										}
								...
								else if (PyroPhase == 3)
								{
									...
									PyroTimer += Time.deltaTime;
									if (PyroTimer > CharacterAnimation["f02_waterPlant_00"].length)
									{
										WillCombust = true;
										PyroPhase++;
									}
							...
							else if (Actions[Phase] == StudentActionType.PrepareFood)
							{
								...
								if (ClubTimer > 60f)
								{
									...
									MyBento.Prompt.enabled = false;
									MyBento.Prompt.Hide();
									ScheduleBlock obj22 = ScheduleBlocks[Phase];
									obj22.destination = "LunchSpot";
									obj22.action = "Eat";
									GetDestinations();
									Pathfinding.target = Destinations[Phase];
									CurrentDestination = Destinations[Phase];
							...
							else if (Actions[Phase] == StudentActionType.PhotoShoot)
							{
								if (StudentManager.Students[19] != null)
								{
									if (StudentManager.Students[19].ClubTimer > 0f && StudentManager.Students[19].DistanceToDestination < 1f)
									{
										if (Male)
										{
											CharacterAnimation["tripodUse_00"].speed = 1f + (float)ClubMemberID * 0.1f;
											CharacterAnimation.CrossFade("tripodUse_00");
										}
										else
										{
											CharacterAnimation["f02_tripodUse_00"].speed = 1f + (float)ClubMemberID * 0.1f;
							...
							else if (Actions[Phase] == StudentActionType.GravurePose)
							{
								if (!Hurry)
								{
									if (SunbathePhase < 2)
									{
										Debug.Log(Name + " believes she has reached this part of their code.");
										if (!StudentManager.CommunalLocker.SteamCountdown && StudentManager.CommunalLocker.CooldownTimer == 0f)
										{
											if (SunbathePhase == 0)
											{
												CharacterAnimation.CrossFade(IdleAnim);
									...
									else
									{
										CharacterAnimation.CrossFade(ClubAnim);
										ClubTimer += Time.deltaTime;
										if (ClubTimer >= CharacterAnimation[ClubAnim].length)
										{
											int clubPhase = ClubPhase;
											while (ClubPhase == clubPhase)
											{
												ClubPhase = UnityEngine.Random.Range(1, GravureAnims.Length);
											}
							...
							{
								if (!Pen.activeInHierarchy)
								{
									Drawing.SetActive(value: true);
									Pen.SetActive(value: true);
								}
								CharacterAnimation.CrossFade(SitAnim);
								if (StudentID == 5 && Clock.GameplayDay == 1 && !BountyCollider.activeInHierarchy)
								{
									BountyCollider.transform.localPosition = new Vector3(0f, 0f, 0f);
									BountyCollider.GetComponent<BoxCollider>().center = new Vector3(0f, 0.65f, 0.175f);
									BountyCollider.GetComponent<BoxCollider>().size = new Vector3(0.45f, 1.3f, 0.55f);
		...
		else
		{
				...
				if (!Dying && !Spraying)
				{
					if (!PinningDown)
					{
						...
						else
						{
							...
							if (base.transform.position.y < -11f || (base.transform.position.y < -10f && base.transform.position.x > 13f))
							{
								Debug.Log(base.name + " just fell below a height of -11, or met the criteria for having officially left the school.");
							...
							else
							{
								...
								else if (Persona == PersonaType.Protective || (Strength == 9 && Chasing))
								{
									if (!Yandere.Dumping && !Yandere.Attacking && !Yandere.Struggling)
									{
										...
										else
										{
											...
											CheckForKnifeInInventory();
											BeginStruggle();
											if (Yandere.EquippedWeapon == null || (Yandere.EquippedWeapon != null && Yandere.EquippedWeapon.Type != WeaponType.Knife))
											{
								...
								else if (Persona == PersonaType.Strict)
								{
									if (!WitnessedMurder)
									{
										...
										else if (ReportPhase == 1)
										{
											...
											if (ReportTimer >= 3f)
											{
												...
												if (MyReporter != null)
												{
													...
													else if (!studentScript3.Teacher)
													{
														if (MyReporter.WitnessedMurder || MyReporter.WitnessedCorpse)
														{
										...
										else if (ReportPhase == 6)
										{
											...
											{
												Debug.Log("As a character was guarding a corpse, the corpse suddenly become concealed.");
												Alarm = 200f;
												FocusOnYandere = true;
									...
									else if (!Yandere.Dumping && !Yandere.Attacking && !Yandere.DelinquentFighting)
									{
										...
										Debug.Log("StudentManager.ChallengeManager.InvincibleRaibaru is: " + StudentManager.ChallengeManager.InvincibleRaibaru);
										if (Strength == 9 && StudentManager.ChallengeManager.InvincibleRaibaru)
										{
											flag8 = false;
										}
										if ((Yandere.Armed && flag8 && Yandere.EquippedWeapon.Type == WeaponType.Knife) || (Yandere.Club == ClubType.MartialArts && Yandere.Armed && Yandere.EquippedWeapon.Type == WeaponType.Knife) || (flag8 && Yandere.Weapon[1] != null && Yandere.Weapon[1].Type == WeaponType.Knife && !Yandere.Weapon[1].Broken) || (flag8 && Yandere.Weapon[2] != null && Yandere.Weapon[2].Type == WeaponType.Knife && !Yandere.Weapon[2].Broken))
										{
							...
							if (CharacterAnimation["f02_electrocution_00"].time > 2f && CharacterAnimation["f02_electrocution_00"].time < 5.9500003f)
							{
								...
								else
								{
									if (!LightSwitch.Flicker)
									{
										...
										GameObject obj23 = UnityEngine.Object.Instantiate(LightSwitch.Electricity, base.transform.position, Quaternion.identity);
										obj23.transform.parent = Bones[1].transform;
										obj23.transform.localPosition = Vector3.zero;
				...
				else
				{
					...
					if (DistanceToDestination < 5f || DistractionTarget.Leaving)
					{
						...
						else if (DistanceToDestination < TargetDistance)
						{
							...
							if (StudentID == StudentManager.SuitorID && StudentManager.DatingMinigame.SuitorAndRivalTalking && !Yandere.CanMove && DistractTimer <= 0f)
							{
								DistractTimer = 1f;
							}
							if (DistractTimer <= 0f)
			...
			if (Hunting)
			{
				...
				{
					GameObject obj24 = UnityEngine.Object.Instantiate(AlarmDisc, base.transform.position + Vector3.up, Quaternion.identity);
					obj24.GetComponent<AlarmDiscScript>().Originator = this;
					obj24.GetComponent<AlarmDiscScript>().Shocking = true;
					obj24.GetComponent<AlarmDiscScript>().NoScream = true;
					obj24.GetComponent<AlarmDiscScript>().Silent = true;
					obj24.GetComponent<AlarmDiscScript>().Slave = true;
					HuntTimer = 0f;
				}
				if (HuntTarget != null)
				{
					...
					if (HuntTarget.Alive && !HuntTarget.Tranquil && !HuntTarget.PinningDown && !HuntTarget.SentHome)
					{
						if (DistanceToDestination > TargetDistance)
						{
							...
							{
								string text9 = "";
								text9 = (Male ? "brokenStandUp_00" : "f02_brokenStandUp_00");
								if (CharacterAnimation[text9].time >= CharacterAnimation[text9].length)
								{
									MurderSuicidePhase++;
									Pathfinding.canSearch = true;
									Pathfinding.canMove = true;
						...
						else
						{
							...
							if (!NEStairs.bounds.Contains(base.transform.position) && !NWStairs.bounds.Contains(base.transform.position) && !SEStairs.bounds.Contains(base.transform.position) && !SWStairs.bounds.Contains(base.transform.position) && !StudentManager.EastGazebo.bounds.Contains(base.transform.position) && !StudentManager.WestGazebo.bounds.Contains(base.transform.position) && !PoolStairs.bounds.Contains(base.transform.position) && !NEStairs.bounds.Contains(HuntTarget.transform.position) && !NWStairs.bounds.Contains(HuntTarget.transform.position) && !SEStairs.bounds.Contains(HuntTarget.transform.position) && !SWStairs.bounds.Contains(HuntTarget.transform.position) && !StudentManager.EastGazebo.bounds.Contains(HuntTarget.transform.position) && !StudentManager.WestGazebo.bounds.Contains(HuntTarget.transform.position) && !PoolStairs.bounds.Contains(HuntTarget.transform.position))
							{
								if (Pathfinding.canMove)
								{
									...
									if (!AttackWillFail)
									{
										...
										SpawnTimeRespectingAudioSource(MurderSuicideSounds, Offset);
										SpawnTimeRespectingAudioSource(MurderSuicideKiller, Offset);
									}
									...
									if (!AttackWillFail)
									{
										...
										SpawnTimeRespectingAudioSource(HuntTarget.MurderSuicideVictim, Offset);
								...
								else
								{
									...
									else if (MurderSuicidePhase == 2)
									{
										...
										{
											GameObject obj25 = UnityEngine.Object.Instantiate(Ragdoll.BloodPoolSpawner.BloodPool, base.transform.position + base.transform.up * 0.021f + base.transform.forward, Quaternion.identity);
											obj25.transform.localEulerAngles = new Vector3(90f, UnityEngine.Random.Range(0f, 360f), 0f);
											obj25.transform.parent = Police.BloodParent;
			...
			if (Suicide)
			{
				string animation = "f02_suicide_00";
				if (Male)
				{
					animation = "suicide_00";
				}
				if (MurderSuicidePhase == 0)
				{
					string text10 = "f02_brokenStandUp_00";
					if (Male)
					{
						text10 = "brokenStandUp_00";
					}
					if (CharacterAnimation[text10].time >= CharacterAnimation[text10].length)
					{
						MurderSuicidePhase++;
						Pathfinding.canSearch = false;
						Pathfinding.canMove = false;
				...
				else if (MurderSuicidePhase == 1)
				{
					...
					if (CharacterAnimation[animation].time >= 11f / 15f)
					{
						MyWeapon.transform.parent = ItemParent;
						MyWeapon.transform.localScale = new Vector3(1f, 1f, 1f);
						MyWeapon.transform.localPosition = Vector3.zero;
				...
				{
					if (CharacterAnimation[animation].time >= 4.1666665f)
					{
						Debug.Log("Stabbed neck!");
				...
				{
					if (CharacterAnimation[animation].time >= 6.1666665f)
					{
				...
				{
					if (CharacterAnimation[animation].time >= 7f)
					{
						Ragdoll.BloodPoolSpawner.SpawnPool(base.transform);
						MurderSuicidePhase++;
					}
				}
				else if (MurderSuicidePhase == 5 && CharacterAnimation[animation].time >= CharacterAnimation[animation].length)
				{
					MyWeapon.transform.parent = null;
					MyWeapon.DoNotRelocate = true;
					MyWeapon.Drop();
			...
			if (Investigating)
			{
				...
				else if (InvestigationPhase == 1)
				{
					...
					if (DistanceToDestination > InvestigationDistance)
					{
						...
						if (Vector3.Distance(base.transform.position, LastPosition) < 0.001f)
						{
							Debug.Log(Name + " has not moved more than .01f since the last time we checked her position.");
							StuckTimer += Time.deltaTime;
							if (StuckTimer > 1f)
							{
								Debug.Log("Supposedly, something was blocking " + Name + " from proceeding while they were trying to investigate a noise.");
								CurrentDestination.position = base.transform.position;
								Pathfinding.target.position = base.transform.position;
								StuckTimer = 0f;
							}
						}
			...
			if (DramaticReaction)
			{
				Debug.Log("DramaticReaction is firing.");
				if (Pathfinding.canMove)
				{
					Pathfinding.canSearch = false;
					Pathfinding.canMove = false;
					if (!Male)
					{
						...
						{
							CharacterAnimation.Play("f02_dramaticFrontal_00");
						}
						else
						{
							CharacterAnimation.Play("f02_dramaticStealth_00");
						}
					}
					else if (!Yandere.AttackManager.Stealth)
					{
						CharacterAnimation.Play("dramaticFrontal_00");
					}
					else
					{
						CharacterAnimation.Play("dramaticStealth_00");
					}
				}
			...
			if (Dodging)
			{
				...
				{
					Debug.Log(Name + " has finished their dodging animation.");
					Dodging = false;
					if (!TurnOffRadio)
					{
						Routine = true;
			...
			{
				Debug.Log("A corpse that was being guarded has been concealed in a trash bag. " + Name + " should now react as if they know the player is a murderer.");
				Alarm = 200f;
				Alarmed = false;
				MurdersWitnessed = 1;
			...
			if (TakingOutTrash)
			{
				...
				{
					if (Mathf.Abs(base.transform.position.y - TrashDestination.position.y) <= 2.5f && Vector3.Distance(base.transform.position, new Vector3(TrashDestination.position.x, base.transform.position.y, TrashDestination.position.z)) < 1f)
					{
						...
						CurrentDestination = Yandere.Incinerator.DumpPointForStudents;
						Pathfinding.target = Yandere.Incinerator.DumpPointForStudents;
						TrashPhase++;
					}
				}
				else if (TrashPhase == 2 && DistanceToDestination < 1f)
				{
	...
	private void LookAtYandere()
	{
		...
		targetRotation = Quaternion.LookRotation(new Vector3(Yandere.transform.position.x, base.transform.position.y, Yandere.transform.position.z) - base.transform.position);
		base.transform.rotation = Quaternion.Slerp(base.transform.rotation, targetRotation, 10f * Time.deltaTime);
		if (FollowTarget != null && !AwareOfMurder && Vector3.Distance(base.transform.position, FollowTarget.transform.position) < 4f && FollowTarget.Attacked && FollowTarget.Alive && !FollowTarget.Tranquil && !Blind)
		{
			Debug.Log("Raibaru should be aware that Osana is being attacked.");
			AwareOfMurder = true;
			Alarm = 200f;
			...
			if (ragdollScript != null && !ragdollScript.Hidden && !ragdollScript.Concealed)
			{
				...
				if (flag && CanSeeObject(collider.gameObject, collider.transform.position, CorpseLayers, Mask))
				{
					...
					if (component != null && !component.Concealed)
					{
						...
						Corpse = component;
						if ((Club == ClubType.Delinquent && Corpse.Student.Club == ClubType.Bully) || (Persona == PersonaType.Fragile && Corpse.Student.Club == ClubType.Bully))
						{
	...
	private void UpdateVisibleClothing()
	{
		ID = 0;
		while (ID < StudentManager.BloodyClothing.Length && BloodPool == null && StudentManager.BloodyClothing[ID] != null)
		{
			Collider myCollider = StudentManager.BloodyClothing[ID].MyCollider;
			if (myCollider != null && Vector3.Distance(base.transform.position, myCollider.transform.position) < VisionDistance && CanSeeObject(myCollider.gameObject, myCollider.transform.position))
			{
				BloodPool = myCollider.transform;
				OriginalBloodPoolLocation = BloodPool.transform.position;
				WitnessedBloodPool = true;
				if (Club != ClubType.Delinquent && StudentManager.BloodReporter == null && !Police.Called && !LostTeacherTrust)
				{
					...
					ReportingBlood = true;
					Debug.Log(Name + " has just appointed themself as a BloodReporter.");
					if (BloodPool.gameObject.GetComponent<PickUpScript>() != null)
					{
						Debug.Log("Attempting to mark the blood pool as ''under investigation''.");
						BloodPool.gameObject.GetComponent<PickUpScript>().UnderInvestigation = true;
					}
					DetermineBloodLocation();
				}
			}
	...
	private void UpdateVision()
	{
		...
		if ((Sleepy || TakingUpAHeadacheSpot) && DistanceToDestination < 10f)
		{
			Debug.Log("Sleepy student is near their destination.");
			flag = true;
		}
		if (!flag)
		{
			...
			if (!WitnessedMurder && !CheckingNote && !Shoving && !Slave && !Struggling && flag2 && !Drownable && !Fighting)
			{
				if (Police.Corpses > 0)
				{
					...
					if (VisibleCorpses.Count > 0)
					{
						if (!WitnessedCorpse)
						{
							...
							if ((Club == ClubType.Delinquent || Persona == PersonaType.Fragile) && Corpse.Student.Club == ClubType.Bully)
							{
				...
				if (VisibleCorpses.Count == 0 && !WitnessedCorpse)
				{
					...
					else if ((StudentID == StudentManager.CurrentID || (Persona == PersonaType.Strict && Fleeing)) && !Wet && !Guarding && !IgnoreBlood && !InvestigatingPossibleDeath && !Spraying && !Emetic && !Threatened && !Sedated && !Headache && !SentHome && !Slave && !Talking && !Confessing && !SentToLocker && !Meeting && !IgnoringThingsOnGround && !RetreivingMedicine && !StudentManager.KokonaTutorial)
					{
						...
						{
							bool flag3 = false;
							PickUpScript[] bloodyClothing = StudentManager.BloodyClothing;
							for (int i = 0; i < bloodyClothing.Length; i++)
							{
								if (bloodyClothing[i] != null)
								{
									flag3 = true;
								}
							}
		...
		else
		{
			...
			if (Distractor.Club == ClubType.Cooking && Distractor.ClubActivityPhase > 0 && Distractor.Actions[Distractor.Phase] == StudentActionType.ClubAction)
			{
				...
				if ((double)CharacterAnimation[PlateEatAnim].time > 6.83333)
				{
					Fingerfood[Distractor.StudentID].SetActive(value: false);
				}
				else if ((double)CharacterAnimation[PlateEatAnim].time > 2.66666)
				{
					Fingerfood[Distractor.StudentID].SetActive(value: true);
				}
			}
			else
			{
	...
	private void UpdateTalkInput()
	{
		if (Prompt.Circle[0].fillAmount == 0f)
		{
			...
			if (!StudentManager.EmptyDemon)
			{
				...
				if (Alarm > 0f || AlarmTimer > 0f || Yandere.Armed || Yandere.Shoved || Stripping || Waiting || InEvent || SentHome || Threatened || Guarding || VisitSenpaiDesk || (Distracted && !Drownable) || (StudentID == 1 && !flag2) || Yandere.YandereVision || TakingOutTrash || Yandere.transform.position.z < -96f)
				{
					if (Alarm > 0f)
					{
			...
			if (Prompt.Circle[0].fillAmount == 0f)
			{
				...
				else if (InEvent || !CanTalk || GoAway || Fleeing || (Meeting && !Drownable) || Wet || TurnOffRadio || InvestigatingBloodPool || (MyPlate != null && MyPlate.parent == RightHand) || flag10 || ReturningMisplacedWeapon || Actions[Phase] == StudentActionType.Bully || Actions[Phase] == StudentActionType.Graffiti || (CanTakeSnack && IgnoreFoodTimer > 0f) || MustTrip || (FollowTarget != null && FollowTarget.InEvent))
				{
					...
					if (Actions[Phase] == StudentActionType.Bully)
					{
						Debug.Log("Can't talk because Bully is true.");
					}
					if (Actions[Phase] == StudentActionType.Graffiti)
					{
						Debug.Log("Can't talk because Graffiti is true.");
					}
					if (IgnoreFoodTimer > 0f)
					{
					...
					else
					{
						...
						if (!Grudge)
						{
							...
							else
							{
								...
								Debug.Log("Opening the Dialogue Wheel from here.");
								DialogueWheel.MouseDelta.x = 0f;
								DialogueWheel.MouseDelta.y = 0f;
								DialogueWheel.HideShadows();
								DialogueWheel.Selected = 0;
		...
		if (!Yandere.Armed && Pushable)
		{
			...
			if (!FocusOnYandere)
			{
				...
				Routine = false;
				Pushed = true;
				if (CurrentDestination == base.transform && base.transform.position.z < 0f)
				{
			...
			{
				Yandere.NotificationManager.CustomText = "Can't push someone who can see you!";
				Yandere.NotificationManager.DisplayNotification(NotificationType.Custom);
			}
			return;
		}
	...
	{
		base.transform.rotation = Quaternion.Slerp(base.transform.rotation, Pathfinding.target.rotation, Time.deltaTime * 10f);
		Alarm -= Time.deltaTime * 100f * (1f / Paranoia);
		EyeShrink = Mathf.Lerp(EyeShrink, 1f, Time.deltaTime * 10f);
	...
	private void UpdateWitnessedMurder()
	{
		...
		else
		{
			...
			Debug.Log(Name + " is now checking if Yandere-chan killed their loved one...");
			if (StudentManager.IronMaiden.Victim != null)
			{
				Yandere.TargetStudent = StudentManager.IronMaiden.Victim;
			}
			if ((Yandere.Attacking && Yandere.TargetStudent != null && LovedOneIsTargeted(Yandere.TargetStudent.StudentID)) || (Yandere.Struggling && Yandere.TargetStudent != null && LovedOneIsTargeted(Yandere.TargetStudent.StudentID)) || (ElectrocutionVictim > 0 && LovedOneIsTargeted(ElectrocutionVictim)) || (Yandere.Carrying && LovedOneIsTargeted(Yandere.CurrentRagdoll.StudentID)) || (Yandere.Dragging && LovedOneIsTargeted(Yandere.CurrentRagdoll.StudentID)) || (PartnerID > 0 && Yandere.Bloodiness > 0f && Yandere.NearestCorpseID == PartnerID) || (StudentManager.IronMaiden.Victim != null && LovedOneIsTargeted(StudentManager.IronMaiden.Victim.StudentID)))
			{
				...
				SprintAnim = OriginalSprintAnim;
				DoNotMourn = true;
				StudentToMourn = Yandere.TargetStudent;
				if (Yandere.TargetStudent != null)
				{
					Corpse = Yandere.TargetStudent.Ragdoll;
				}
			}
	...
	private void UpdateAlarmed()
	{
		...
		if (!Threatened)
		{
			...
			else if (!Reacted)			# If ((AlarmTimer == 5f) & (!Reacted))
			{
				if (Teacher)
				{
					if (!WitnessedCorpse)
					...
					if (!Chasing && !Attacked && ((YandereVisible && Concern == 5) || Yandere.Noticed))
					{
				...
				else if (Club != ClubType.Delinquent)
				{
					...
					else if (Witnessed == StudentWitnessType.Corpse)
					{
						...
						else if (InCouple && Corpse.StudentID == PartnerID)
						{
							Subtitle.CustomText = "No...NO...!";
							Subtitle.UpdateLabel(SubtitleType.Custom, 0, 5f);
						}
					...
					{
						Debug.Log(Name + " witnessed ''Interruption''?");
						if (!StudentManager.Eighties)
						{
							if (StudentID == 11)
							{
						...
						else if (!StudentManager.CustomMode)
						{
							if (StudentID > 10 && StudentID < 21)
							{
								Subtitle.UpdateLabel(SubtitleType.RivalInterruptionReaction, StudentID - 10, 5f);
							}
							else
							{
								Subtitle.UpdateLabel(SubtitleType.InterruptionReaction, 2, 5f);
			...
			if (((Club == ClubType.Council && !DoNotShove) || Shovey) && DistanceToPlayer < 1.1f && !Yandere.Invisible && !EatingSnack && (Yandere.Armed || ((Yandere.Carrying || Yandere.Dragging) && !Yandere.CurrentRagdoll.Concealed) && Prompt.InSight)
			{
		...
		if (!Injured)
		{
			...
			if (DistanceToPlayer > 1f && Patience > 0)
			{
				...
				else
				{
					...
					if (ThreatTimer < 0f)
					{
						...
						FocusOnStudent = FocusOnYandere = Distracted = Threatened = Alarmed = NoTalk = false;
						Routine = true;
						IgnoreTimer = 5f;
						AlarmTimer = 0f;
					}
			...
			if (!NoTalk)
			{
				...
				if (StudentManager.CombatMinigame.Delinquent == null)
				{
					...
					if (((Yandere.Carrying || Yandere.Dragging) && !Yandere.CurrentRagdoll.Concealed))
					{
						Yandere.EmptyHands();
					}
					if (Yandere.Pickpocketing)
					...
					Yandere.NearSenpai = Yandere.Degloving = Yandere.CanMove = FocusOnStudent = FocusOnYandere = false;
					Yandere.GloveTimer = 0f;
					Distracted = Fighting = true;
					ThreatTimer = 0f;
					...
					SpeechLines.Stop();
					SpawnAlarmDisc();
					if (WitnessedMurder || WitnessedCorpse || (!StudentManager.CombatMinigame.Practice))
					{
						Subtitle.Speaker = this;
						...
						else		# When (!StudentManager.CombatMinigame.Practice)
						{
				...
				{
					Threatened = Alarmed = base.enabled = false;
				}
				return;
			}
			ThreatTimer -= Time.deltaTime;
			if (ThreatTimer < 0f)
			{
				...
				Distracted = Threatened = Alarmed = Notalk = false;
				Routine = true;
				IgnoreTimer = 5f;
				AlarmTimer = 0f;
			}
		...
		if (Yandere.CanMove)
		{
			if (!WitnessedMurder && !WitnessedCorpse)
			{
				Distracted = Threatened = false;
				EndAlarm();
			}
			else
			{
				Threatened = Alarmed = Injured = false;
				PersonaReaction();
			}
		}
	}
	...
	private void UpdateSplashed()
	{
		...
		if (LightSwitch == null)
		{
			...
			Subtitle.Speaker = this;
			Subtitle.UpdateLabel(SplashSubtitleType, newParam, 5f);
			SplashPhase++;
			if (!Male)
			{
				CurrentDestination = Pathfinding.target = StudentManager.StrippingPositions[GirlID];
			...
			{
				CurrentDestination = Pathfinding.target = StudentManager.MaleStripSpot;
			}
		}
		else if (!LightSwitch.BathroomLight.activeInHierarchy)
		{
			...
			CurrentDestination = Pathfinding.target = LightSwitch.ElectrocutionSpot;
			Pathfinding.speed = WalkSpeed;
			BathePhase = -1;
			InDarkness = true;
		}
	...
	private void UpdateTurningOffRadio()
	{
		if (Radio.On || (RadioPhase == 3 && Radio.transform.parent == null))
		{
			...
			else if (RadioPhase == 2)
			{
				...
				else if (Persona == PersonaType.Protective || Hurry)
				{
					Debug.Log("Sprinting because Protective.");
					CharacterAnimation.CrossFade(RunAnim);
					Pathfinding.speed = 4f;
	...
	private void UpdateVomiting()
	{
		...
		else if (VomitPhase == 2)
		{
			if (StudentManager.KokonaTutorial)
			{
				if ((CharacterAnimation[VomitAnim].time > 13f) || (CharacterAnimation[VomitAnim].time < 1f))
				{
		...
		else if (VomitPhase == 3)
		{
			if (CharacterAnimation[VomitAnim].time >= CharacterAnimation[VomitAnim].length)
			{
				...
				if (Male)
				{
					...
					Pathfinding.target = CurrentDestination = StudentManager.MaleWashSpot;
				}
				else
				{
					StudentManager.GetFemaleWashSpot(this);
					Pathfinding.target = CurrentDestination = StudentManager.FemaleWashSpot;
				}
				if (VomitDoor != null)
				{
					VomitDoor.Prompt.enabled = VomitDoor.enabled = true;
				}
				Pathfinding.canSearch = Pathfinding.canMove = true;
				Pathfinding.speed = WalkSpeed;
				DistanceToDestination = 100f;
		...
		else if (VomitPhase == 5 && CharacterAnimation[WashFaceAnim].time > CharacterAnimation[WashFaceAnim].length)
		{
			...
			GetDestinations();
			Phase++;
			Pathfinding.target = CurrentDestination = Destinations[Phase];
	...
	private void StopVomitting()
	{
		...
		Prompt.Label[0].text = "     Talk";
		Pathfinding.canSearch = Pathfinding.canMove = true;
		Distracted = Drownable = Vomiting = Private = false;
		CanTalk = Routine = true;
		Emetic = false;
		VomitPhase = 0;
		StudentManager.UpdateMe(StudentID);
		WalkAnim = OriginalWalkAnim;
	...
	private void UpdateConfessing()
	{
		if (StudentID > 1 && StudentID != StudentManager.SuitorID)
		{
			...
			else if (ConfessPhase == 3)
			{
				...
				{
					CurrentDestination = Pathfinding.target = StudentManager.RivalConfessionSpot;
					Pathfinding.canSearch = Pathfinding.canMove = true;
					Pathfinding.speed = 4f;
			...
			else if (ConfessPhase == 4)
			{
				if (DistanceToDestination < 0.5f)
				{
					if (StudentManager.Eighties && StudentID == 19)
					{
						StudentManager.FollowPrimaryLookSecondary.gameObject.SetActive(value: true);
					}
					CharacterAnimation.CrossFade(IdleAnim);
					Pathfinding.canSearch = Pathfinding.canMove = false;
					ConfessPhase++;
		...
		else if (ConfessPhase == 1)
		{
			...
			if (CharacterAnimation[text3].time >= CharacterAnimation[text3].length)
			{
				...
				CurrentDestination = Pathfinding.target = StudentManager.SuitorConfessionSpot;
				Pathfinding.canSearch = Pathfinding.canMove = true;
				Pathfinding.speed = 4f;
				Note.SetActive(value: false);
				CharacterAnimation.CrossFade(SprintAnim);
	...
	private void UpdateMisc()
	{
		...
		if (!Fleeing)
		{
			...
			{
				if ((base.transform.position.y < -11f || (base.transform.position.y < -10f && base.transform.position.x > 13f)) && StudentID > 1 && Phase > 1)
				{
					base.gameObject.SetActive(value: false);
				}
				return;
			...
			if (((Club == ClubType.Council && !DoNotShove) || (Shovey && !Following)) && !WitnessedSomething)
			{
				if (DistanceToPlayer < 5f)
				{
					...
					if (Mathf.Abs(Vector3.Angle(-base.transform.forward, Yandere.transform.position - base.transform.position)) <= 45f && Yandere.Stance.Current != StanceType.Crouching && Yandere.Stance.Current != StanceType.Crawling && Yandere.CanMove && !Yandere.Invisible && (Yandere.h != 0f || Yandere.v != 0f) && (Yandere.Running || DistanceToPlayer < 2f))
					{
						...
						Alarm = 200f;
						TemporarilyBlind = FocusOnYandere = true;
						Routine = false;
						Pathfinding.canSearch = false;
						Pathfinding.canMove = false;
					}
	...
	private void LateUpdate()
	{
		...
		if (!Male)
		{
			...
			{
				Cosmetic.RightTemple.localScale = new Vector3(0f, 0f, 1f);
				Cosmetic.LeftTemple.localScale = new Vector3(0f, 0f, 1f);
			}
		}
		...
		{
			Debug.Log("Uh-oh! For some reason, Student #" + StudentID + ", " + Name + ", had a null CurrentDestination.");
			Debug.Log(Name + "'s action was meant to be " + ScheduleBlocks[Phase].action + " and their destination was meant to be " + ScheduleBlocks[Phase].destination);
			FixNullDestination();
		}
		...
		if (Chesster)
		{
			Debug.Log("Jaw is: " + Jaw.transform.localEulerAngles.z);
			Debug.Log("Jaw Delta is: " + (Jaw.transform.localEulerAngles.z - 40.46975f));
			ChessterJaw.transform.localEulerAngles = new Vector3(Jaw.transform.localEulerAngles.z - 40.46975f, 0f, 0f);
		}
	}

	public void FixNullDestination()
	{
		if (Phase > 0)
		{
			Debug.Log("Student #" + StudentID + ", " + Name + ", is now running FixNullDestination().");
			Debug.Log(Name + "'s current Phase is:" + Phase);
		}
		if (ScheduleBlocks[Phase].destination == "Sunbathe" && SunbathePhase > 0)
		{
			Destinations[Phase] = StudentManager.SunbatheSpots[0];
		}
		if (Destinations[Phase] == null)
		{
	...
	public void AttackReaction()
	{
		...
		if (StudentID != 97 && StudentManager.Students[97] != null && !StudentManager.Students[97].Hunted && StudentManager.Students[97].transform.position.z < -47f && base.transform.position.z < -47f && base.transform.position.x > -14f && base.transform.position.x < 14f)
		{
			...
			StudentManager.Students[97].AwareOfMurder = true;
			StudentManager.Students[97].VisionBonus = 100f;
			StudentManager.Students[97].Alarm = 200f;
		}
		...
		if (Following)
		{
			Debug.Log("Student was ''Following'' at the time of the attack.");
			StudentManager.TranqDetector.TranqCheck();
		}
		if (!Male)
		{
		...
		else if (Strength == 9 && !Emetic && !Lethal && !Sedated && !Headache && !Yandere.CanTranq)
		{
			...
			else
			{
				...
				Yandere.ShoulderCamera.OverShoulder = false;
				Yandere.ShoulderCamera.Counter = false;
				Yandere.RPGCamera.enabled = false;
				Yandere.Senpai = base.transform;
				Yandere.Attacking = true;
				Yandere.CanMove = false;
		...
		if (StudentManager.AlternateTimeline)
		{
	...
	public void SenpaiNoticed()
	{
		...
		for (int i = 0; i < 101; i++)
		{
			if (StudentManager.Students[i] != null && StudentManager.Students[i].SeekingMedicine)
			{
				StudentManager.Students[i].GoSitInInfirmary();
			}
		}
	}
	...
	private void WitnessMurder()
	{
		...
		{
			WitnessedBloodyWeapon = WitnessedBloodPool = WitnessedSomething = WitnessedWeapon = WitnessedLimb = Fleeing = false;
			ReportPhase = 0;
		}
		...
		SpecialRivalDeathReaction = SenpaiWitnessingRivalDie = ReturningMisplacedWeapon = InvestigatingBloodPool = CameraReacting = FocusOnStudent = FocusOnYandere = TakingOutTrash = Investigating = Distracting = EatingSnack = Threatened = Distracted = Reacted = Routine = NoTalk = Shy = Wet = false;
		WitnessedMurder = Alarmed = true;
		...
		if (Club == ClubType.Delinquent || Persona == PersonaType.Fragile)
		{
			...
			if (!StudentManager.Eighties && Yandere.PickUp != null && Yandere.PickUp.BodyPart != null && Yandere.PickUp.BodyPart.StudentID > 80 && Yandere.PickUp.BodyPart.StudentID < 86)
			{
		...
		else
		{
			...
			if (Talking)
			{
				...
				ParticleSystem.EmissionModule emission = Hearts.emission;
				Pathfinding.canSearch = Pathfinding.canMove = true;
				Obstacle.enabled = Talk.enabled = Talking = Waiting = emission.enabled = false;
				StudentManager.EnablePrompts();
			}
	...
	private void ChaseYandere()
	{
		...
		CurrentDestination = Pathfinding.target = Yandere.transform;
		Pathfinding.speed = 5f;
	...
	private void PersonaReaction()
	{
		...
		if (Persona == PersonaType.Lovestruck)
		{
			Debug.Log("Because " + Name + " is Lovestruck, she is doing a quick check to see if Senpai is unavailable or dead.");
			if (StudentManager.Students[LovestruckTarget] != null && StudentManager.Students[LovestruckTarget].SenpaiWitnessingRivalDie)
			{
				Debug.Log(Name + " can't run and tell Senpai, because Senpai is busy mourning a rival's corpse.");
				Persona = PersonaType.Loner;
			}
			else if (StudentManager.Students[LovestruckTarget] != null && !StudentManager.Students[LovestruckTarget].gameObject.activeInHierarchy)
			{
				Debug.Log(Name + " can't run and tell Senpai, because Senpai is disabled for some reason.");
				Persona = PersonaType.Loner;
			}
		}
		if (!DoNotMourn && InCouple && Corpse.Student.StudentID == PartnerID)
		{
			Debug.Log(Name + " is in a couple and just found the corpse of their partner.");
			Persona = PersonaType.Lovestruck;
			StudentToMourn = Corpse.Student;
			LovestruckTarget = PartnerID;
		}
		if (Persona == (PersonaType.Loner || PersonaType.Spiteful))
		{
			...
			if (Club == ClubType.Delinquent)
			{
				...
				var subtitleType = SubtitleType.DelinquentFlee;
				
				if (Injured && WitnessedMurder)
				{
					Debug.Log("You won't get away with what you've done!");
					
					subtitleType = SubtitleType.DelinquentInjuredFlee;
				}
				else if (FoundFriendCorpse)
				{
					subtitleType = SubtitleType.DelinquentFriendFlee;
				}
				else if (FoundEnemyCorpse)
				{	
					subtitleType = SubtitleType.DelinquentEnemyFlee;
				}

				Subtitle.Speaker = this;
				Subtitle.UpdateLabel(subtitleType, 1, 3f);
		...
		else if (Persona == PersonaType.TeachersPet)
		{
			...
			else
			{
				...
				else if (WitnessedCorpse)
				{
					if (Club == ClubType.Council)
					{
						Subtitle.CustomText = "How did this happen?!";
						Subtitle.UpdateLabel(SubtitleType.Custom, 0, 5f);
					}
					else
					{
		...
		else if (Persona == PersonaType.SocialButterfly)
		{
			...
			if (StudentManager.NEStairs.bounds.Contains(StudentManager.HidingSpots.List[StudentID].position) || StudentManager.NWStairs.bounds.Contains(StudentManager.HidingSpots.List[StudentID].position) || StudentManager.SEStairs.bounds.Contains(StudentManager.HidingSpots.List[StudentID].position) || StudentManager.SWStairs.bounds.Contains(StudentManager.HidingSpots.List[StudentID].position))
			{
				StudentManager.HidingSpots.List[StudentID].position = new Vector3(0f, 0f, -6f);
			}
		...
		else if (Persona == PersonaType.Lovestruck)
		{
			...
			bool flag = false;
			if (!WitnessedMurder && Corpse != null && ((StudentID == 1 && Corpse.Student.Rival) || Corpse.Student == FollowTarget || (InCouple && Corpse.Student.StudentID == PartnerID) || (!StudentManager.Eighties && StudentID == 11 && Corpse.StudentID == StudentManager.ObstacleID) || (!StudentManager.Eighties && StudentID > 1 && StudentID < 4 && Corpse.StudentID > 1 && Corpse.StudentID < 4)))
			{
				flag = true;
			}
			if (flag)
		...
		else if (Persona == PersonaType.PhoneAddict)
		{
			...
			if (!StudentManager.Eighties)
			{
				if (MaiHair != null)
				{
					...
					MaiHair.m_Gravity = Vector3.zero;
				}
		...
		for (ID = 1; ID < JSON.Students[StudentID].ScheduleBlocks.Length; ID++)
		{
			...
			else if (scheduleBlock.destination == "Shock")
			{
				...
				else
				{
					...
					{
						Destinations[ID] = StudentManager.ShockedSpots[0];
					}
				}
			}
			...
			else if (scheduleBlock.destination == "LockerRoom")
			{
				...
				{
					Destinations[ID] = StudentManager.FemaleLockerRoomChangingSpot;
				}
			}
			...
			else if (scheduleBlock.destination == "MorningRivalWitnessSpot")
			{
				if (!MorningRivalWitness)
				{
					Destinations[ID] = StudentManager.MorningRivalWitnessSpots[StudentManager.MorningRivalWitnesses];
					StudentManager.MorningRivalWitnesses++;
					MorningRivalWitness = true;
				}
			}
			else if (scheduleBlock.destination == "LunchRivalWitnessSpot")
			{
				if (!LunchRivalWitness)
				{
					Destinations[ID] = StudentManager.LunchRivalWitnessSpots[StudentManager.LunchRivalWitnesses];
					StudentManager.LunchRivalWitnesses++;
					LunchRivalWitness = true;
				}
			}
			else if (scheduleBlock.destination == "AfterRivalWitnessSpot" && !AfterRivalWitness)
			{
				Destinations[ID] = StudentManager.AfterRivalWitnessSpots[StudentManager.AfterRivalWitnesses];
				StudentManager.AfterRivalWitnesses++;
				AfterRivalWitness = true;
			}
			...
			else if (scheduleBlock.action == "Socialize")
			{
				if (StudentID == 26)
				{
					Debug.Log("One of this character's Actions was set to ''Socializing.'' This may produce a problem.");
				}
			...
			else if (scheduleBlock.action == "DeskDraw")
			{
				Actions[ID] = StudentActionType.DeskDraw;
			}
		}
	}
	...
	public void GetWet()
	{
		...
		else if (DyedBrown)
		{
			...
			if (!StudentManager.Eighties && StudentID == 5)
			{
	...
	public void ChangeSchoolwear()
	{
		...
		{
			if (Cosmetic.EightiesClubAccessories[StudentID] != null)
			{
				Cosmetic.EightiesClubAccessories[StudentID].SetActive(value: false);
			}
			if (Club == ClubType.Sports && Male)
			{
				...
				MyRenderer.materials[1].mainTexture = Cosmetic.Trunks[StudentID];
				MyRenderer.materials[2].mainTexture = Cosmetic.FaceTexture;
				if (Cosmetic.SkinColor > 0)
				{
					MyRenderer.materials[0].mainTexture = Cosmetic.SkinTextures[Cosmetic.SkinColor];
				}
			}
			else
			{
		...
		else if (Schoolwear == 3)
		{
			...
			if (!Male)
			{
				Debug.Log(Name + ", a female, is putting on a gym uniform.");
				if (!StudentManager.Eighties && StudentID > 80 && StudentID < 86)
				{
					Debug.Log(Name + " is a bully and needs to have tan skin.");
					Cosmetic.SkinColor = 6;
				}
				MyRenderer.materials[0].mainTexture = Cosmetic.SkinTextures[Cosmetic.SkinColor];
	...
	public void GoChange()
	{
		...
		{
			Debug.Log(Name + "'s school uniform is unavailable. They should change into a gym uniform, instead.");
			MustChangeClothing = true;
		}
		Distracted = false;
	}
	...
	public void ChangeClubwear()
	{
		if (!ClubAttire)
		{
			...
			else if (Club == ClubType.MartialArts)
			{
				...
				if (Armband != null)
				{
					Armband.transform.localPosition = new Vector3(-0.1f, -0.019f, 0.01f);
					Armband.transform.localScale = new Vector3(1.33333f, 1.33333f, 1.33333f);
				}
			}
			else if (Club == ClubType.Science)
			{
			...
			{
				if (OriginalClub != ClubType.Sports)
				{
					return;
				}
				Cosmetic.EightiesGoggles[StudentID].SetActive(value: false);
				if (Clock.Period < 3 || StudentManager.PoolClosed)
				{
					Debug.Log("A Sports Club student is now changing clothing...");
					if (StudentManager.Eighties)
					{
						GymTexture = EightiesGymTexture;
					}
					...
					{
						MyRenderer.materials[0].mainTexture = GymTexture;
						MyRenderer.materials[2].mainTexture = GymTexture;
						MyRenderer.materials[1].mainTexture = Cosmetic.FaceTexture;
						MyRenderer.materials[2].mainTexture = Cosmetic.SkinTextures[Cosmetic.SkinColor];
					}
					else
				...
				{
					Cosmetic.EightiesGoggles[StudentID].SetActive(value: true);
				}
				if (Cosmetic.EightiesClubAccessories[StudentID] != null)
				{
					Cosmetic.EightiesClubAccessories[StudentID].SetActive(value: false);
	...
	{
		Debug.Log(Name + " just fired CameraReact()");
		CharacterAnimation.cullingType = AnimationCullingType.AlwaysAnimate;
		Pathfinding.canSearch = false;
		Pathfinding.canMove = false;
		Obstacle.enabled = true;
	...
	{
		Debug.Log("The player is allegedly attacking a student with an ID of: " + yandereTargetID);
		...
		if (StudentManager.Eighties)
		{
			...
			{
				bool flag2 = false;
				if (Club != ClubType.Council && Club != ClubType.Delinquent)
				{
			...
			if (StrongReactionID > 0)
			{
				...
				if (StrongReactionID > 0)
				{
					result = yandereTargetID == StrongReactionID;
				}
				return result;
			}
			return false;
		...
		bool flag4 = StudentID == 3 && yandereTargetID == 2;
		bool flag5 = StudentID == 2 && yandereTargetID == 3;
		bool flag6 = StudentID == 11 && yandereTargetID == 10;
		bool flag7 = StudentID == 6 && yandereTargetID == 11;
		bool flag8 = StudentID == 21 && yandereTargetID == 12;
		bool flag9 = StudentID == 38 && yandereTargetID == 37;
		bool flag10 = StudentID == 37 && yandereTargetID == 38;
		bool flag11 = StudentID == 30 && yandereTargetID == 25;
		bool flag12 = StudentID == 25 && yandereTargetID == 30;
	...
	public void GoCommitMurder()
	{
		...
		{
			CharacterAnimation.CrossFade("brokenStandUp_00");
	...
	public void ChameleonCheck()
	{
		...
		if (Yandere != null)
		{
			Debug.Log("Yandere.AnimSetID is " + Yandere.AnimSetID + " and Student's AnimSetID is " + AnimSetID);
			Debug.Log("Yandere.Persona is: " + Yandere.Persona);
			if (StudentManager.Eighties && !StudentManager.CustomMode && StudentID == 20)
			{
				flag = true;
			}
	...
	private void EndAlarm()
	{
		...
		if (WalkSpeed == 4f)
		{
			Debug.Log("After reaching the end of EndAlarm, " + Name + "'s WalkSpeed was 4.");
		}
		else
		{
			Debug.Log("After reaching the end of EndAlarm, " + Name + "'s WalkSpeed was: " + WalkSpeed);
		}
		if (EatingSnack)
		{
			EatingSnack = false;
	...
	public void GetSleuthTarget()
	{
		...
		if (SleuthTargetsSought < 101)
		{
			...
			if (SleuthID < 98)
			{
				...
				if (StudentManager.Students[SleuthID].Slave)
				{
					GetSleuthTarget();
					return;
				}
			...
			{
				bool flag = false;
				if (StudentManager.LockerRoomArea.bounds.Contains(Yandere.transform.position) || StudentManager.MaleLockerRoomArea.bounds.Contains(Yandere.transform.position) || StudentManager.EastBathroomArea.bounds.Contains(Yandere.transform.position) || StudentManager.WestBathroomArea.bounds.Contains(Yandere.transform.position))
				{
					flag = true;
				}
	...
	public void GetFoodTarget()
	{
		...
		if (SleuthID < 90)
		{
			...
			else if (StudentManager.Students[SleuthID].FollowTarget != null)
			{
				GetFoodTarget();
			}
	...
	public void UpdateAnimLayers()
	{
		...
		else
		{
			...
			CharacterAnimation["f02_casualWave_04"].layer = 10;
			CharacterAnimation["f02_casualWave_04"].weight = 0f;
			CharacterAnimation["friendWave_00"].layer = 9;
			CharacterAnimation["friendWave_00"].weight = 0f;
			if (!StudentManager.Eighties && StudentID == 37)
			{
	...
	public void DetermineWhatWasWitnessed()
	{
		...
		else if (YandereVisible)
		{
			...
			else if ((Yandere.Laughing && Yandere.LaughIntensity > 15f) || (StudentID > 1 && Yandere.Stance.Current == StanceType.Crouching) || Yandere.Stance.Current == StanceType.Crawling || Yandere.SuspiciousActionTimer > 0f || (Yandere.WearingRaincoat && !flag8) || Yandere.Lockpicking || (Yandere.Carrying && Yandere.CurrentRagdoll.Concealed) || (Yandere.Dragging && Yandere.CurrentRagdoll.Concealed) || (Yandere.Schoolwear == 2 && Yandere.transform.position.z < 30f) || (AnnoyedByRadio > 1 && Yandere.PotentiallyAnnoyingTimer > 0f) || (Yandere.PreparingThrow && Yandere.Obvious) || (Yandere.PickUp != null && Yandere.PickUp.CleaningProduct && Clock.Period != 5 && Yandere.CleaningNotSuspicious == 0f))
			{
				Debug.Log(Name + " witnessed weird behavior.");
				if (AnnoyedByRadio > 1 && Yandere.AnnoyingGiggleTimer > 0f)
				{
					Debug.Log(Name + " witnessed the player repeatedly turning on a loud radio.");
				}
			...
			{
				Debug.Log(Name + " witnesed insane laughter.");
				Witnessed = StudentWitnessType.Insanity;
				RepLoss = 10f;
				Concern++;
			}
			...
			{
				Debug.Log("We don't know what " + Name + " witnessed!!");
			}
	...
	public void DetermineTeacherSubtitle()
	{
		...
		if (Guarding && YandereVisible)
		{
			...
			else if (Yandere.Sanity < 66.66666f || (Yandere.WearingRaincoat && !flag) || Yandere.Lockpicking)
			{
				Witnessed = StudentWitnessType.Insanity;
			}
		}
	...
	{
		Debug.Log(Name + " is running RaibaruCannotFindOsana()");
		SpeechLines.Stop();
		CharacterAnimation.CrossFade(LookLeftRightAnim);
		SnackTimer += Time.deltaTime;
		if (SnackTimer > 5f)
	...
	{
		Debug.Log("Current rival - " + Name + " - just fired PlaceBag() and put their bookbag on their desk.");
		if (!StudentManager.Eighties)
		{
		...
		{
			Debug.Log("Attempting to update rival's routine to whatever their Custom Mode routine is supposed to be.");
			ScheduleBlock obj2 = ScheduleBlocks[2];
			obj2.destination = OriginalDestination;
			obj2.action = OriginalAction;
			if (obj2.action == "Sleuth")
			{
				BecomeSleuth();
			}
		}
	...
	public void BecomeSleuth(bool DoNotAdjustSchedule = false)
	{
		if (!SleuthInitialized)
		{
			SleuthInitialized = true;
	...
	public void BlendOutOfSittingAnim()
	{
		...
		{
			Debug.Log("...who the hell has a null CharacterAnimation? Apparently, it's " + base.gameObject.name);
		}
	}
	...
	public void Oversleep()
	{
		if (StudentID != 15 && ScheduleBlocks.Length == 10)
		{
			...
			obj2.destination = "InfirmaryBed";
			obj2.action = "Sleep";
			obj2.time = 99999f;
			ScheduleBlock obj3 = ScheduleBlocks[8];
			obj3.destination = "InfirmaryBed";
			obj3.action = "Sleep";
			obj3.time = 99999f;
			ScheduleBlock obj4 = ScheduleBlocks[9];
			obj4.destination = "InfirmaryBed";
			obj4.action = "Sleep";
			obj4.time = 99999f;
		}
	}
	...
	public void ChangeClothing()
	{
		...
		Debug.Log("SchoolwearUnavailable is: " + SchoolwearUnavailable);
		Debug.Log("ChangeClothingPhase is: " + ChangeClothingPhase);
		...
		else if (ChangeClothingPhase == 2 && CharacterAnimation[StripAnim].time >= CharacterAnimation[StripAnim].length)
		{
			CurrentAction = Actions[Phase];
			...
			{
				Debug.Log(Name + " should be going to class now.");
	...
	public void CheckForNearbyWalls()
	{
		...
		if (TooCloseToWall)
		{
			Debug.Log("Firing TeleportToBlackCube() from here, specifically.");
			TeleportToBlackCube();
		}
	}

	public void TeleportToBlackCube()
	{
		Debug.Log("Teleporting the protagonist and " + Name + " to the black cube in the sky.");
		...
		Yandere.MainCamera.transform.position = new Vector3(0.5f, 101.5f, -1f);
		Yandere.MainCamera.transform.eulerAngles = new Vector3(0f, 0f, 0f);
		if (Yandere.ShoulderCamera.Timer >= 6f)
		{
			Debug.Log("Shoulder camera was 6 or more...");
			CharacterAnimation["f02_moCounterB_00"].time = Yandere.ShoulderCamera.Timer;
			Yandere.CharacterAnimation["f02_moCounterA_00"].time = Yandere.ShoulderCamera.Timer;
		}
		Physics.SyncTransforms();
	}
	...
	{
		if (OriginalBloodMaterial == null)
		{
			OriginalBloodMaterial = BloodMaterial;
		}
		if (StudentManager.CensorBlood)
		{
			BloodMaterial = CensorMaterial;
			if (LiquidProjector.material == OriginalBloodMaterial)
			{
				LiquidProjector.material = BloodMaterial;
			}
		}
		...
		{
			BloodMaterial = OriginalBloodMaterial;
			if (LiquidProjector.material == CensorMaterial)
			{
				LiquidProjector.material = BloodMaterial;
			}
		}
